#!/bin/bash
# Script para configuração automática do Access Point Motorola AP5131

export http_proxy=""

if [ -z $1 ]
then
	echo Informe o quarto byte do IP. Ex: Caso o IP seja 192.168.0.63, use: $0 63
	exit 1
...
echo "set email lsupnpdu@ucs.br" >> $arquivo
...
echo "set ipadr 1 200.233.31.188" >> $arquivo
echo "set ipadr 2 200.233.31.189" >> $arquivo
...
echo "add v1v2c netucscom ro 1.3.6.1" >> $arquivo
...
echo "set domain 1 ucs.br" >> $arquivo
...
echo ""
echo "Abrindo firefox para cadastramento completo..."
firefox "https://cmdb.example.com/swmanager/cadastraap.php?Id_Unidade=$Id_Unidade&Nome=AP$loc$predio$andar$numero&IP=$ip&MAC_LAN=$mac_lan&MAC_WAN=$mac_wan&MAC_RADIO1=$mac_radio1&MAC_RADIO2=$mac_radio2&Patrimonio=$patrimonio&Id_status=1&Marca=Motorola&Modelo=AP5131&&Serial=000&Descricao=\"$descricao\"" & 
echo ""
